...
本次版本以界面重做为主线，覆盖 7 个面板视图：

| 面板 | 说明 |
|------|------|
| 边沿触发球 | 页面右侧常驻触发入口 |
| 提取中状态 | 提取进度与阶段反馈 |
| 摘要面板 | AI 摘要主视图，支持模板切换与会话式修改 |
| 图片面板 | 网格缩略图预览、批量选择、ZIP 打包 |
| 表格面板 | 表格结构化预览与 CSV 导出 |
| 编辑面板 | Markdown 所见即所得编辑 |
| 设置面板 | AI 服务商配置、API Key 加密存储、规则管理 |

设计上做了三处收敛：配色改为低饱和暖调浅色体系（正文 `#faf9f5`，强调色 `#c96442`），标题启用衬线字体（Newsreader / Lora）以强化长文阅读观感；原先的 emoji 图标（🔮 🔒 🚫 👁️ 📤）全部替换为内联 SVG，消除跨平台渲染差异；面板结构从扁平堆叠改为分层容器，为后续扩展预留槽位。
...
提取链路：内容脚本捕获 → 规则引擎解析 → AI 适配层摘要 → 导出器落地，全程在浏览器内完成，不经过第三方服务器。
...
| [plugin/README.md](plugin/README.md) | 插件本体说明与加载指引 |
